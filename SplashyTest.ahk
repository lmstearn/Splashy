...
;https://webapps.stackexchange.com/questions/162310/url-image-filtering-url-suffixes-and-wikimedia
	...
	*/
	...
	SplitPath % This.imagePath,,, spr
		...
		{
		This.PicInScript := {}      
		This.SetCapacity(This.PicInScript, 21748 << !!A_IsUnicode)
		; png format wroks well
...
gui, Test: +resize -caption -DPIScale +E%WS_EX_NOINHERITLAYOUT% HWNDthisHWnd
...
gui, Test: add, pic, section x0 y0, % GetData("bg")

	loop
	{
		loop
		{
		gui, Test: add, pic, x+0, % GetData("bg")
		} Until (A_Index * splashPicWd >= 17 * xSep)

	if (A_Index * splashPicHt >= 12 * ySep)
	Break
	gui, Test: add, pic, x0 y+0, % GetData("bg")
	}
		...
		else
		{
			loop, 44 ; number of Splashy control variables
			{
				...
				spr := c . "_" . r
					...
					GuiControl, Test:, %spr%, % GetData("BTONSAV")
				}
			}
		}
...
vRepaintSplashy := 1
	...
	; This probably posts a WM_PAINT- will we get there first?
		if (!(Splashy.vOnTop))
		{
		WinSet, AlwaysOnTop, 1, % "ahk_id" . Splashy.hWnd()
		WinSet, AlwaysOnTop, 0, % "ahk_id" . Splashy.hWnd()
		}
...
; Rapid clicks on "Click to Repaint" or even "Click to Launch" repro.
; Something internal is off- a timer might be a better workaround

if (Instr(clipboard, "HICON:*") || clipboard == "Click to Repaint" || clipboard == "Click to Launch")
clipBoard := clipSaved
else
clipSaved := clipBoard
...
i := c + 4 * (r - 1)
	...
	if (!ctlTogs[i] && %out%)
	GuiControl, Test:, %A_Guicontrol%, % GetData("BTONSAV")
	else
	GuiControl, Test:, %A_Guicontrol%, % %out% ? GetData("BTON") : GetData("BTOFF")
...
GetCtrlType(i)
{
	switch i
	{
	case 5, 6, 25, 26, 27, 37, 38, 39:
	return 0
	case 4, 6, 15, 16, 21, 33:
	return -1
	Default:
	return 1
	}
...
Static Colors := [0x00FF00, 0xFF0000, 0xFF00FF]
	switch i
	{
		case 1, 3, 4, 8, 9, 10, 11, 12, 14, 30, 31, 32, 42, 43, 44:
		return 1
		case 7, 22, 28, 34, 40:
		{
		...
		case 15, 16:
		{
		InputBox, textIn, Please enter %textIn%,Left`, top`, co-ordinates for Splashy in the screen or parent.`n`nA `(`"c"`) applied to either vPosX`, vPosY co-ordinate field`ncentres the Splashy image on the appropriate axes.`nA `(`"l"`) or null `(`""`) retains the co-ordinate's last position.
			if (Errorlevel)
			return "**Errorlevel**"
		}
		case 17, 18:
		...
		case 21, 33:
		{
		InputBox, textIn, Please enter %textIn%, Text to appear above or below the image.
			if (textIn == "" || Errorlevel)
			return "**Errorlevel**"
		}
		case 26, 38:
